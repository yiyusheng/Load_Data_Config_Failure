...
failRecordA <- fread(file.path(dir_datatext,'硬盘故障单140101_141231A.txt'),sep = '~',header = F,encoding = 'UTF-8')
...
failRecordB <- fread(file.path(dir_datatext,'硬盘故障单140101_141231B.txt'),sep = '\t',encoding = 'UTF-8')
failRecordB$f_time <- as.POSIXct(failRecordB$createtime,tz = 'UTC')

#S3. deduplicate for A[cancel]
...
#S3.save
save(failRecordA,failRecordB,file = file.path(dir_data,'failRecord2014.Rda'))
