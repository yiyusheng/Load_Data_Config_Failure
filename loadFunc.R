...
dedupTime <- function(data.flist,dayDup,attr,attr_time = 'f_time'){
  data.flist <- data.flist[order(data.flist[[attr]],data.flist[[attr_time]]),]
  data.flist[[attr]] <- as.character(data.flist[[attr]])
  delset <- numeric()
  
  pSvrid <- data.flist[[attr]][1]
  pFtime <- data.flist[[attr_time]][1]
  for (i in 2:nrow(data.flist)){
    curSvrid <- data.flist[[attr]][i]
    curFtime <- data.flist[[attr_time]][i]
    if (curSvrid == pSvrid & 
